...
class measure{
    ...
    public static function add() {
		...
		if(isset($_FILES['img']) && $_FILES['img']['error'] == 0 && $_FILES['img']['size']  > 0){
			...
			$data['img'] = utils::upload(time().'.'.$ext, $_FILES['img']['tmp_name'], '../upload/measure/', ['jpg', 'jpeg', 'png']);
		}
		...
		if(empty($response['errors'])){
			...
			if ($data_id) {
				CMS::log([
					...
					'subj_id' => $data_id,
					'action' => 'add',
					'descr' => 'measure '.$data_id.' added by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
	...
	public static function edit($id) {
		...
		if(isset($_FILES['img']) && $_FILES['img']['error'] == 0 && $_FILES['img']['size']  > 0){
		    ...
		    $_df = UPLOADS_DIR.'measure/'.$data['img'];
		...
		if(empty($response['errors'])){
			...
			if ($updated) {
				CMS::log([
					...
					'subj_id' => $id,
					'action' => 'edit',
					'descr' => 'measure'.$id.' edited by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
    ...
    public static function delete($id){
		...
		if(isset($img['img'])){
			unlink(UPLOADS_DIR.'measure/'.$img['img']);
		}
		...
		if ($deleted) {
			...
			CMS::log([
				...
				'subj_id' => $id,
				'action' => 'delete',
				'descr' => 'measure deleted by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
			]);
		}
	}
		...
		foreach($items as $k=>$item){
			...
			if($sort){
				CMS::log([
					...
					'subj_id' => $item,
					'action' => 'ajax_sort',
					'descr' => 'measure sorted by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
			}
