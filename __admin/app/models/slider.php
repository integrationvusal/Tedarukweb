...
class slider{
	...
	public static function add() {
		...
		if(isset($_FILES['img']) && $_FILES['img']['error'] == 0 && $_FILES['img']['size']  > 0){
			...
			$data['img'] = utils::upload(time().'.'.$ext, $_FILES['img']['tmp_name'], '../upload/slider/', ['jpg', 'jpeg', 'png']);
		}
		
		if(empty($data['img']))		$response['errors'][] = 'slider_add_err_img_empty';
	...
	public static function edit($id) {
		...
		if(isset($_FILES['img']) && $_FILES['img']['error'] == 0 && $_FILES['img']['size']  > 0){
			$ext = pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);
			$_df = UPLOADS_DIR.'slider/'.$data['img']; 
			if(file_exists($_df)) unlink($_df);
			$data['img'] = utils::upload(time().'.'.$ext, $_FILES['img']['tmp_name'], '../upload/slider/', ['jpg', 'jpeg', 'png']);
		}
	...
	public static function delete($id){
		...
		$img = '../upload/slider/'.CMS::$db->get("SELECT img FROM `".self::$tbl."` WHERE id = :id LIMIT 1", [':id'=>$id]);
