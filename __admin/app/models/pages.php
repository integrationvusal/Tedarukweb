...
class pages {
	...
	public static function getPagesList() {
		...
		$where = ["published = true", "lang_id = ".CMS::$db->escape(1)];
		if (!empty($_GET['q'])) {
		...
		$c = CMS::$db->select('COUNT(id)', self::$pages_tbl, $where);
		self::$items_amount = $c;
		...
		if ($pages_amount>0) {
			...
			$list = CMS::$db->selectAll('*', self::$pages_tbl, $where, 'ordering', $start_from, self::$pp);
			//print "<pre>RESULT:\n".var_export($list, 1)."\n\nQUERIES:\n".var_export(CMS::$db->queries, 1)."\n\nERRORS:\n".var_export(CMS::$db->errors, 1)."\n</pre>";
		}
	...
	public static function validateUserData($id) { // 2016-11-15
	...
	public static function saveUserData($id, $data) { // 2016-11-15
		...
		if (empty($response['errors'])) {
			...
			if ($id) {
				...
				if ($data['lang']!=$_SESSION[CMS::$sess_hash]['ses_adm_lang']) {
	...
	public static function editUser($id) { // 2016-05-24
		$response = [
			'success' => false,
			'message' => '',
			'data' => [],
