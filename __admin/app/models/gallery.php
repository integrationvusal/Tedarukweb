...
class gallery {
	...
	public static function addGallery() {
		...
		if (empty($response['errors'])) {
			...
			$gallery['category'] = ';'.implode(';', $_POST['cats']).';';
			...
			if (!empty($gallery_id)) {
				...
				foreach ($translates as $lang=>$tr_data) {
                    ...
                    if(empty($dirname) && !empty($tr_data['name']))
			            $dirname = $tr_data['name'];
	...
	public static function editGallery($gallery_id) { 
		...
		$gallery = [];
		$translates = [];
		$old_dir = self::$dir.$_POST['old_dir'];
		...
		if (empty($response['errors'])) {
			...
			$gallery['category'] = isset($_POST['cats'])?(';'.implode(';', $_POST['cats']).';'):'';
			...
			if(is_dir($old_dir))
                rename($old_dir, self::$dir.$dirname);
