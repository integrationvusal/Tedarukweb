...
class markets{
    ...
    public static function add() {
		...
		if(isset($_FILES['img']) && $_FILES['img']['error'] == 0 && $_FILES['img']['size']  > 0){
			...
			$data['img'] = utils::upload(time().'.'.$ext, $_FILES['img']['tmp_name'], '../upload/markets/', ['jpg', 'jpeg', 'png']);
		}
		...
		if(empty($response['errors'])){
			...
			if ($data_id) {
				CMS::log([
					...
					'subj_id' => $data_id,
					'action' => 'add',
					'descr' => 'markets '.$data_id.' added by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
	...
	public static function edit($id) {
		...
		if(isset($_FILES['img']) && $_FILES['img']['error'] == 0 && $_FILES['img']['size']  > 0){
		    ...
		    $_df = UPLOADS_DIR.'markets/'.$data['img'];
		...
		if(empty($response['errors'])){
			...
			if ($updated) {
				CMS::log([
					...
					'subj_id' => $id,
					'action' => 'edit',
					'descr' => 'markets'.$id.' edited by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
    ...
    public static function delete($id){
		...
		if(isset($img['img'])){
			unlink(UPLOADS_DIR.'markets/'.$img['img']);
		}
		...
		if ($deleted) {
			...
			CMS::log([
				...
				'subj_id' => $id,
				'action' => 'delete',
				'descr' => 'markets deleted by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
			]);
		}
	}
		...
		foreach($items as $k=>$item){
			...
			if($sort){
				CMS::log([
					...
					'subj_id' => $item,
					'action' => 'ajax_sort',
					'descr' => 'markets sorted by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
			}
