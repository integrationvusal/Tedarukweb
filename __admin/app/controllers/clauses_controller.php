...
class clauses_controller extends controller {

	private static $runtime = [];

	public static function action_list() { // 2016-12-04
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_clauses_list');
		...
		$params['news'] = news::getNewsList('clauses');
		...
		$params['canWrite'] = CMS::hasAccessTo('clauses/list', 'write');
		...
		return self::render('clauses_list', $params);
	}

	public static function action_add() { 
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_clauses_add');

		$params = [];

		$params['canWrite'] = CMS::hasAccessTo('clauses/add', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=clauses&action=list': $_GET['return']);
		...
		if (isset($_POST['add'])) {
			$params['op'] = news::addNews('clauses');
		...
		return self::render('clauses_add', $params);
	}

	public static function action_edit() { // 2016-01-15
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_clauses_edit');

		$params = [];

		$params['canWrite'] = CMS::hasAccessTo('clauses/edit', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=clauses&action=list': $_GET['return']);
		...
		return self::render('clauses_edit', $params);
	}
	...
	public static function action_delete() {
		...
		$params = [];
		$params['canWrite'] = CMS::hasAccessTo('clauses/delete', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=clauses&action=list': $_GET['return']);
	...
	public static function action_ajax_set_status() { // 2016-12-04
		...
		if (!CMS::hasAccessTo('clauses/ajax_set_status', 'write')) {
			$response['code'] = '403';
	...
	public static function action_ajax_sort() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('clauses/ajax_sort', 'write')) {
			$response['code'] = '403';
	...
	public static function action_ajax_paged_sort() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('clauses/ajax_paged_sort', 'write')) {
			$response['code'] = '403';
			$response['message'] = 'ajax_request_not_alllowed_to_write';
		} else if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest') {
	...
	public static function action_ajax_delete_image() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('clauses/ajax_delete_image', 'write')) {
			$response['code'] = '403';
			$response['message'] = 'ajax_request_not_alllowed_to_write';
		} else if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest') {
