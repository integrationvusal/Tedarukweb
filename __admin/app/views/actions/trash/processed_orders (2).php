...
<div class="content-box">
    ...
    <div class="content-box-content">
        ...
        $query=$pdo->query("SELECT * FROM `cs_orders` WHERE `status`='9' ");
        ...
        if($setir>0){
            ...
            <table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="tablesorter">
                ...
                <tr>
                    <th id="order_proc_id">ID</th>
                    <th id="order_proc_num">Номер заказа</th>
                    <th id="order_proc_date">Дата поступления</th>
                    <th id="order_proc_cost">Стоимость</th>
                    <th id="order_proc_title">Метка</th>
                    <th>Управление</th>
                </tr>
                ...
                for ($j=$start; $j<$end; $j++){
                    if($j==$page){
                        ...
                        $row = $pdo->query("SELECT * FROM `cs_orders` WHERE `status`='9' ORDER BY `id` DESC LIMIT ".($count*$j)." ,".($count)."")->fetchAll();
        ...
        <script type="text/javascript">
            $(document).ready(function()
            {
                ...
                var row_id         = $('#order_proc_id');          // set cookoe value equal to 0
                var row_number     = $('#order_proc_num');         // set cookoe value equal to 1
                var row_date       = $('#order_proc_date');        // set cookoe value equal to 2
                var row_cost       = $('#order_proc_cost');        // set cookoe value equal to 3
                var row_title      = $('#order_proc_title');       // set cookoe value equal to 4

                row_id.click(function(){
                    $.cookie('cookie_order_sort_row', '0', { expires: 7 });
                });

                row_number.click(function(){
                    $.cookie('cookie_order_proc_sort_row', '1', { expires: 7 });
                });

                row_date.click(function(){
                    $.cookie('cookie_order_proc_sort_row', '2', { expires: 7 });
                });

                row_cost.click(function(){
                    $.cookie('cookie_order_proc_sort_row', '3', { expires: 7 });
                });

                row_title.click(function(){
                    $.cookie('cookie_order_proc_sort_row', '4', { expires: 7 });
                });
                ...
                $("#myTable").tablesorter({
                    ...
                    sortList: [[$.cookie('cookie_order_proc_sort_row'),0]],
                    headers:{
                        5: {sorter: false }
                    }
                }); // sorter end
