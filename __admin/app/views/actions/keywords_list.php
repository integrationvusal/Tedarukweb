	...
	/*$("#sortable").sortable({
		...
		update: function(e, ui){
			...
			$.ajax({
	            ...
	            type: 'POST',
	            url: '<?=view::create_url('keywords', 'sort')?>',
...
// ]]>
	...
	<form action="<?=view::create_url('keywords', 'delete') ?>" method="post" id="formDeleteItem">
	...
	<section class="contextual-navigation">
		<nav>
			<?php if (CMS::hasAccessTo('keywords/add', 'write')) { ?>
			<a href="<?=view::create_url('keywords', 'add', ['return' => $link_return]) ?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_keywords_add');?></a>
			<?php } ?>
		</nav>
	</section>
	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header with-border">
				<h3 class="box-title"><?=CMS::t('keywords_list_details', [
					'{count}' => $count,
				...
				<div class="box-tools pull-right col-sm-5 col-lg-6">
					<form action="" method="get" id="formSearchAndFilter">
						...
						<div class="input-group has-feedback">
							<input type="text" name="q" value="<?=utils::safeEcho(@$_GET['q'], 1);?>" placeholder="<?=CMS::t('keywords_search');?>" class="form-control input-md" onfocus="this.value='';" />
            ...
            <div class="box-body">
				...
				<table class="table table-bordered table-striped">
					...
					<tbody id="sortable" data-inc-count="<?=$curr_pg?>">
						...
						?>
							<tr id="keyword-<?=$data['id']?>">
								<td><?=$data['keyword']?></td>
								<td>
									<?php if (CMS::hasAccessTo('keywords/delete', 'write')) { ?>
