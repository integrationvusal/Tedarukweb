						...
						?>
							<tr>
								<td>
									...
									<?=view::gravatar($user['login'], 40, ['class' => 'userpic']);?>
									<?php } else { ?>
									<img src="<?=UPLOADS_DIR;?>avatars/cms_users/<?=$user['avatar'];?>" alt="" class="userpic" />
									<?php } ?>
									<?=(($user['id']==$_SESSION[CMS::$sess_hash]['ses_adm_id'])? ('<strong>'.$user['login'].'</strong>'): $user['login']);?>
								</td>
								<td>
									<a href="?controller=cms_users&amp;action=edit&amp;id=<?=$user['id'];?>&amp;return=<?=$link_return;?>&amp;<?=time();?>" title="">
										<?=(($user['id']==$_SESSION[CMS::$sess_hash]['ses_adm_id'])? ('<strong>'.$user['name'].'</strong>'): $user['name']);?>
									</a>
								</td>
								<td>
									<?=CMS::t($user['role']);?>
								...
								<td>
									<?=utils::formatMySQLDate('d.m.Y H:i:s', $user['reg_date']);?>
								</td>
								<td>
								...
								<td>
									...
									<script type="text/javascript">
										utils.setConfirmation('click', '#aDeleteItem_<?=$user['id'];?>', '<?=CMS::t('delete_confirmation');?>', function($el) {
