					...
					<tbody id="tablesorter" data-page="<?=$current-1?>" data-limit="<?=$limit?>">
						<?php
							foreach ($alldata as $data) {
							    $data['link_img'] = '/img/no_video.png';
								$_link=SITE.'uploads/videos/'.$data['link'];
							    if($data['link'] && strpos($data['link'], 'v=') !== false){
    							    $data['link_img'] = explode('v=', $data['link']);
    							    $_link = $data['link_img'] = 'http://img.youtube.com/vi/'.$data['link_img'][1].'/mqdefault.jpg';
							    }
						?>
							<tr data-id="<?=$data['id']?>">
								...
								<td><?=$_link?></td>
								<td>
									<?=utils::safeEcho($data['selected_cats'], true) ?>
								</td>
								<td><?=$data['title']?></td>
								<td>
