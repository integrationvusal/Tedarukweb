$(document).ready(function(){	    
	...
	$('.new-folder').click(function(){
		...
		$.ajax({
			  ...
			  data: {csrf:$('#csrf').val(),path: folder_path, path_thumb: folder_path_thumb}
			}).done(function( msg ) {
			window.location.href = $('#refresh').attr('href') + '&' + new Date().getTime();
		});
		}
...
function apply_img(file,type_file,external){
	top.opener.CKEDITOR.tools.callFunction(getCookie('funcnum'), $('#cur_dir').val()+file);
	window.close();
/*
    if ($('#popup').val()==1) var window_parent=window.opener; else var window_parent=window.parent;
    var path = $('#cur_dir').val();
...
function delete_file(file1,file2) {
    $.ajax({
	    type: "POST",
	    url: "delete_file.php",
	    data: {csrf:$('#csrf').val(), path: file1, path_thumb: file2}
	}).done(function( msg ) {
    });
}
...
function delete_folder(folder1,folder2) {
    $.ajax({
	    ...
	    data: {csrf:$('#csrf').val(), path: folder1, path_thumb: folder2}
	}).done(function( msg ) {
    });
}
